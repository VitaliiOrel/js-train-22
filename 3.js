...
class Store {
   ...
   */
  unsubscribe(cust) {
    this.customers = this.customers.filter(customer => customer !== cust)
  }
  /**
  ...
  createProduct(name) {
    new Product(name)
    this.customers.forEach(cust => cust.sendMessage('new product' + name))
  }
  /**
   ...
   */
  sendEmail(product) {
    this.customers.forEach(cust => console.log(`Новий продукт ${product.name}" в магазині ${this.name}!`))
  }
